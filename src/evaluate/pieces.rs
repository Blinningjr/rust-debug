...
use gimli::{
    ...
    Expression, Reader, Register, Unit, UnitOffset,
};
    ...
    loop {
        let resolved = match result {
            ...
            RequiresRegister {
                ...
                resolve_requires_reg(unit, &mut eval, &mut result, register, base_type, registers)?
            }
            ...
            RequiresAtLocation(die_ref) => resolve_requires_at_location(
                dwarf,
                unit,
                pc,
                &mut eval,
            ...
            RequiresParameterRef(unit_offset) => resolve_requires_paramter_ref(
                dwarf,
                unit,
                &mut eval,
                &mut result,
            ...
            RequiresRelocatedAddress(num) => {
                resolve_requires_relocated_address(&mut eval, &mut result, num)?
            }

            RequiresIndexedAddress { index, relocate: _ } => {
                resolve_requires_indexed_address(dwarf, unit, &mut eval, &mut result, index)?
            }

            RequiresBaseType(unit_offset) => {
                resolve_requires_base_type(unit, &mut eval, &mut result, unit_offset)?
            }
        };
...
fn resolve_requires_at_location<R: Reader<Offset = usize>, T: MemoryAccess>(
    dwarf: &Dwarf<R>,
    unit: &Unit<R>,
    pc: u32,
 ...
 * TODO: Check and test if correct.
 */
fn resolve_requires_mem<R: Reader<Offset = usize>, T: MemoryAccess>(
    unit: &Unit<R>,
    eval: &mut Evaluation<R>,
    result: &mut EvaluationResult<R>,
    address: u64,
    ...
    R: Reader<Offset = usize>,
{
    match registers.get_register_value(&reg.0) {
        ...
        None => Ok(EvalResult::RequiresRegister { register: reg.0 }),
    }
}

fn resolve_requires_entry_value<R: Reader<Offset = usize>, T: MemoryAccess>(
    dwarf: &Dwarf<R>,
    unit: &Unit<R>,
    eval: &mut Evaluation<R>,
    result: &mut EvaluationResult<R>,
    ...
    R: Reader<Offset = usize>,
{
    ...
    *result =
        eval.resume_with_entry_value(convert_to_gimli_value(match entry_value.to_value() {
            Some(val) => val,
...
fn resolve_requires_paramter_ref<R: Reader<Offset = usize>, T: MemoryAccess>(
    dwarf: &Dwarf<R>,
    unit: &Unit<R>,
    eval: &mut Evaluation<R>,
    result: &mut EvaluationResult<R>,
...
fn resolve_requires_indexed_address<R: Reader<Offset = usize>>(
    dwarf: &Dwarf<R>,
    unit: &Unit<R>,
    eval: &mut Evaluation<R>,
    result: &mut EvaluationResult<R>,
